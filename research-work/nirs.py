...
def preprocess_data(data_in):
    ...
    scaler = MinMaxScaler()
    scaled_data = scaler.fit_transform(data_out[['cap-diameter', 'stem-height', 'stem-width']])
    data_out[['cap-diameter', 'stem-height', 'stem-width']] = scaled_data
    
    return data_out, data_out['class']
...
n_range_list = list(range(0, 21, 5))
if not neighbor_slider in n_range_list:
    n_range_list.append(neighbor_slider)
    sorted(n_range_list)
